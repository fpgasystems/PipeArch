...
void train_thread(
    ...
    mf_parameter param)
{
    model = mf_train_with_validation(tr, va, param);
}
...
int main(int argc, char **argv)
{
    ...
    if(!option.on_disk)
    {
        try
        {
            cout << "option.tr_path: " << option.tr_path << endl;

            tr[0] = read_problem("syn1.libmf");
            tr[1] = read_problem("syn2.libmf");
            tr[2] = read_problem(option.tr_path);

            for (unsigned j = 0; j < 3; j++) {
                cout << "------------j: " << j << endl;
                cout << "tr.m: " << tr[j].m << endl;
                cout << "tr.n: " << tr[j].n << endl;
                cout << "tr.nnz: " << tr[j].nnz << endl;
            }
    ...
    // {
        ...
        for (unsigned i = 0; i < 4; i++) {
            for (unsigned j = 0; j < 3; j++) {
                cthreads[j] = new thread(train_thread, model[j], &tr[j], &tr[j], option.param);
            }
            for (unsigned j = 0; j < 3; j++) {
                cthreads[j]->join();
                delete cthreads[j];
